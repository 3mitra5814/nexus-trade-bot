...
**網格機器人控制中心，面向需要交易量、自動化和風險可視化、但不想逐筆盯單的交易者。合約為預設模式；主流中心化交易所均支援現貨網格。**

[![Go](https://img.shields.io/badge/Go-1.26%2B-00ADD8?logo=go&logoColor=white)](https://go.dev/)
[![License](https://img.shields.io/badge/license-GPL--3.0-green)](../../LICENSE)
[![One Command](https://img.shields.io/badge/install-one%20command-blue)](#一鍵安裝)
[![Languages](https://img.shields.io/badge/languages-11-orange)](#語言)
...
如果你是普通用戶，建議先用一鍵安裝啟動網頁控制台，只新增「允許交易、禁止提現」的 API，然後用小資金建立測試機器人，確認邏輯和風險都符合預期後再放大。

如果你是開發者，建議從原始碼建置開始，先看 `config.example.yaml`，執行 `go test ./...`，再依需要用 worker 模式載入指定設定檔。

這份 README 的閱讀順序是：安裝、支援交易所、功能說明、策略例子、參數說明、手動安裝、實盤前檢查。
...
- 列印清晰的存取提示區塊，包含本機 URL、監聽位址、PID 檔案和日誌路徑；除非明確綁定公開位址，否則預設不開放遠端存取。
...
- 新增交易所 API，並在驗證通過後再使用。
- 為不同交易對、帳戶和方向建立多個機器人。
- 選擇期貨或現貨。預設選擇期貨。
- 合約支援做多、做空或中性模式；現貨僅支援做多模式。
- 自動載入 Binance、Bitget、Bybit、OKX、Gate 和 Hyperliquid 現貨交易對。
- 即時觀察餘額、交易量、機器人狀態和損益。
- 暫停機器人，更改參數，然後使用最新設定重新啟動。
- 讓風險監控器在市場異常波動時停止交易。

它面向重視執行效率、交易量和風險控制的交易者，而不是讓使用者整天手動編輯設定檔。
...
假設 ETH 從 `3000` 附近開始，然後跌至 `2700`。做多網格通常會承受浮動虧損，因為它會在下跌過程中分層買入；但這些低位買入也會降低平均成本。如果價格隨後從 `2700` 反彈至 `2850`，帳戶可能比單次在 `3000` 買入更早接近盈虧平衡。
...
重要的一點是：網格收益來自準備，而不是樂觀。在放大倉位之前，請先計算市場可能向不利方向移動多遠、機器人最多會累積多少倉位，以及如果市場遲遲不反彈會發生什麼。


## 內建風險保護

快速單邊下跌是激進做多網格最不友善的環境。nexus-trade-bot 內建市場風險監控，用來降低這種風險：
...
對高流動性交易對使用較小間隔和可控訂單金額。目標是高換手率和可預期執行。費率在這裡很重要，因此應盡量使用低費率交易對或手续费折扣方案。

### 2. 市場回調後的做多網格

在市場出現明顯回調後啟動，而不是追逐急漲行情。機器人會分層買入，並在反彈中逐步賣出。這種風格需要足夠保證金來承受更深回調。

### 3. 幣安現貨網格

當你希望機器人買賣真實現貨資產，而不是開槓桿合約倉位時，請使用現貨模式。現貨模式僅做多：機器人會先在低位買入，再在反彈中賣出庫存。它比合約簡單，但仍需要足夠的計價幣餘額，並提前規劃長期下跌的情況。
...
經典網格僅支援合約中性模式。它不設定上下區間，而是即時維持目前網格價下方 50 個買單、上方 50 個賣單，總目標為 100 個活躍掛單。成交後會自動補單，讓買一和賣一盡量保持設定間隔。Hyperliquid 合約目前不支援此模式，因為它不提供所需的中性雙向持倉行為。
...
| `mode` | `normal`、`aggressive` 或 `classic` | `classic` 為固定 50 買 / 50 賣經典網格，會強制使用合約 + 中性方向，並以 100 個活躍掛單為目標。 |
| `direction` | `long`、`short` 或 `neutral` | 做多網格需要為回撤預留保證金。啟動時已有交易所倉位會恢復為機器人庫存。 |
