...
| `trading.adopt_existing_position` | 既存の取引所ポジションをボット在庫として採用するか | デフォルトは `false` です。手動の Bitget ベースポジションはグリッド在庫として扱われず、グリッドの決済注文で誤って閉じられません。既存ポジションを意図的にボットへ管理させたい場合だけ有効化してください。 |
...
- Bitget 先物については、最初に小さいサイズでテストし、ボットの方向、位置モード、および `trading.adopt_existing_position` が希望のものと一致することを確認します。
