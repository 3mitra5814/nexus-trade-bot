...
- ローカル URL、バインドアドレス、PID ファイル、ログパスを含むアクセス案内ブロックを表示します。明示的に公開バインドしない限り、リモートアクセスは既定で無効です。
...
流動性の高いシンボルでは、狭い間隔と制御された注文サイズを使用します。目標は、予測可能な実行による高い売上高です。ここでは手数料率が非常に重要なので、可能な場合は低手数料ペアまたはメイカー手数料割引を使用してください。
...
クラシックグリッドは先物専用のニュートラルモードです。上下レンジを設定せず、現在のグリッド価格の下に買い注文を50本、上に売り注文を50本維持します。合計100本のアクティブ注文を目標にし、約定したグリッドは自動で補充されます。Hyperliquid futures は必要なニュートラル両建て挙動を提供していないため、現時点ではこのモードに対応していません。
...
| `mode` | `normal`、`aggressive`、または `classic` | `classic` は futures + neutral を強制し、50本の買い注文と50本の売り注文、合計100本のアクティブ注文を目標にします。 |
