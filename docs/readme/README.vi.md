...
- In khối truy cập rõ ràng với URL cục bộ, địa chỉ bind, tệp PID và đường dẫn log. Truy cập từ xa mặc định tắt trừ khi bạn bind địa chỉ công khai.
...
Lưới cổ điển chỉ dùng cho futures ở chế độ neutral. Bot giữ 50 lệnh mua đang hoạt động bên dưới giá grid hiện tại và 50 lệnh bán bên trên, không đặt vùng giá trên/dưới cố định. Mục tiêu là tổng cộng 100 lệnh hoạt động; các grid đã khớp sẽ được bổ sung tự động. Hyperliquid futures hiện chưa hỗ trợ chế độ này vì cần hành vi hedge neutral hai chiều.
...
| `mode` | `normal`, `aggressive` hoặc `classic` | `classic` buộc futures + neutral và nhắm 100 lệnh hoạt động: 50 lệnh mua và 50 lệnh bán. |
