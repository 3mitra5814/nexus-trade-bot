...
type binanceAdapter interface {
	GetName() string
	PlaceOrder(context.Context, *binance.OrderRequest) (*binance.Order, error)
	BatchPlaceOrders(context.Context, []*binance.OrderRequest) ([]*binance.Order, bool)
	CancelOrder(context.Context, string, int64) error
	BatchCancelOrders(context.Context, string, []int64) error
	GetOrder(context.Context, string, int64) (*binance.Order, error)
	GetOpenOrders(context.Context, string) ([]*binance.Order, error)
	GetAccount(context.Context) (*binance.Account, error)
	GetPositions(context.Context, string) ([]*binance.Position, error)
	GetBalance(context.Context, string) (float64, error)
	StartOrderStream(context.Context, func(interface{})) error
	StopOrderStream() error
	GetLatestPrice(context.Context, string) (float64, error)
	StartPriceStream(context.Context, string, func(float64)) error
	StartKlineStream(context.Context, []string, string, func(interface{})) error
	StopKlineStream() error
	GetHistoricalKlines(context.Context, string, string, int) ([]*binance.Candle, error)
...
func (w *binanceWrapper) PlaceOrder(ctx context.Context, req *OrderRequest) (*Order, error) {
	...
	return &Order{
		...
		ClientOrderID: utils.RemoveBrokerPrefix("binance", binanceOrder.ClientOrderID),
...
func (w *binanceWrapper) BatchPlaceOrders(ctx context.Context, orders []*OrderRequest) ([]*Order, bool) {
	...
	for i, ord := range binanceResult {
		result[i] = &Order{
			OrderID:       ord.OrderID,
			ClientOrderID: utils.RemoveBrokerPrefix("binance", ord.ClientOrderID),
			Symbol:        ord.Symbol,
			Side:          Side(ord.Side),
